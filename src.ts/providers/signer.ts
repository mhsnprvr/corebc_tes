...
/**
 *  A Signer represents an account on the Core Blockchain, and is most often
...
export interface Signer extends Addressable, ContractRunner, NameResolver {
     ...
     *  - populates ``energyLimit`` via ``signer.estimateEnergy(tx)``
     *  - populates ``networkId`` via ``signer.provider.getNetwork()``
     *  - populates ``type`` and relevant fee data for that type (``energyPrice``
     *    for legacy transactions, ``maxFeePerEnergy`` for EIP-1559, etc)
     *
    ...
    /**
     *  Estimates the required energy required to execute //tx// on the Blockchain. This
     *  will be the expected amount a transaction will require as its ``energyLimit``
     ...
     *  is in flux, which could affect transaction energy requirements.
     *
     *  @throws UNPREDICTABLE_ENERGY_LIMIT A transaction that is believed by the node to likely
     *          fail will throw an error during energy estimation. This could indicate that it
     *          will actually fail or that the circumstances are simply too complex for the
     *          node to take into account. In these cases, a manually determined ``energyLimit``
     *          will need to be made.
     */
    estimateEnergy(tx: TransactionRequest): Promise<bigint>;
