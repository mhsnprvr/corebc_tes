...
export class AbstractProvider implements Provider {
    ...
    async estimateEnergy(_tx: TransactionRequest): Promise<bigint> {
        let tx = this._getTransactionRequest(_tx);
        if (isPromise(tx)) { tx = await tx; }
        return getBigInt(await this.#perform({
            method: "estimateEnergy", transaction: tx
        }), "%response");
    }
        ...
        assert (attempt < MAX_CCIP_REDIRECTS, "CCIP read exceeded maximum redirections", "OFFCHAIN_FAULT", {
         ...
         try {
             ...
             if (!this.disableCcipRead && isCallException(error) && error.data && attempt >= 0 && blockTag === "latest" && transaction.to != null && dataSlice(error.data, 0, 4) === "0x556f1830") {
                 ...
                 const tx = {
                     to: txSender,
                     data: concat([ ccipArgs.selector, encodeBytes([ ccipResult, ccipArgs.extraData ]) ]) as string
                 };
    ...
    async getTransactionReceipt(hash: string): Promise<null | TransactionReceipt> {
        ...
        if (params.energyPrice == null && params.effectiveEnergyPrice == null) {
            ...
            if (tx == null) { throw new Error("report this; could not find tx or effectiveEnergyPrice"); }
...
function encodeBytes(datas: Array<BytesLike>): string {
    ...
    return concat(result) as string;
}
