...
describe("Tests Provider Call Exception", function() {
    ...
    for (const { code, reason } of panics) {
        for (const method of [ "call", "estimateEnergy" ]) {
            for (const providerName of providerNames) {
                ...
                it(`tests panic code: ${ providerName }.${ method }.${ reason }`, async function() {
                    ...
                    const tx = { to: testAddr, data } as TransactionRequest;
                    try {
                        const result = await (method === "call" ? provider.call(tx): provider.estimateEnergy(tx));
                        console.log(result);
