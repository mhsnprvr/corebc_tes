...
describe("Test Transaction Signing", function () {
    ...
    for (const test of tests) {
        it(`tests signing a legacy transaction: ${test.name}`, async function () {
            ...
            const txData = Object.assign({}, test.transaction, { type: 0, accessList: undefined });
            // Use the testcase sans the networkId for a legacy test
            // @ts-ignore
            if (txData.networkId != null && parseInt(txData.networkId) != 0) {
                txData.networkId = "0x00";
            }
    ...
    for (const test of tests) {
        it(`tests signing a Berlin transaction: ${test.name}`, async function () {
            const wallet = new Wallet(test.privateKey);
            const txData = Object.assign({}, test.transaction, { type: 1 });
            const signed = await wallet.signTransaction(txData);
...
//# sourceMappingURL=_test-wallet.js.map