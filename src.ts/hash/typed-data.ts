...
import { sha256, SignatureLike } from "../crypto/index.js";
...
function hexPadRight(value: BytesLike): string {
    ...
    if (padOffset) {
        return concat([ bytes, padding.slice(padOffset) ]) as string;
    }
        ...
        if (fields) {
            ...
            return (value: Record<string, any>) => {
                const values = fields.map(({ name, type }) => {
                    ...
                    if (this.#fullTypes.has(type)) { return sha256(result); }
                    return result;
                });
 ...
 */
export function verifyTypedData(domain: TypedDataDomain, types: Record<string, Array<TypedDataField>>, value: Record<string, any>, signature: SignatureLike, prefix: string): string {
